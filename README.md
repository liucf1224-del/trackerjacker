...
### 大概了解后的理解
1. 前置条件：“必须连接过Wi-Fi并被记录”，才能后续找到吗？
不完全是，但这样效果最好。 分两种情况：

情况A：你想找“特定身份的设备”（比如张三的手机）
你需要曾经获取过它的真实MAC地址，最可靠的场景就是它连接过你家Wi-Fi时，你通过trackerjacker或路由器后台看到了它的真实MAC。

拿到这个真实MAC后，后续只要它再次连接你家Wi-Fi，或者在某些情况下（比如它连接别的网络时）进行通信，你就能再次认出它。

但是，如果它只是打开了Wi-Fi但从未连接任何网络，现代手机广播的探测请求会使用随机MAC，这个随机MAC每隔几分钟就会变一次，你无法把它和“张三的手机”关联起来。

情况B：你只想发现“周围有没有活的手机”（不关心是谁的）
完全不需要任何先验信息。只要对方的Wi-Fi开关开着，它就会广播探测请求，trackerjacker就能看到一串随机MAC。这在灾害废墟场景下有用，可以知道“这里有电子设备在活动”，但无法区分是张三还是李四。

结论：
“先连接被你记录”只是获取真实MAC身份的手段，不是发现设备的必要条件。但如果你的目标是识别并持续追踪某个特定人，那获取真实MAC就是必须的，而最方便的途径就是他连过你监控的网络。
简单来说就是这玩意开wifi你能知道mac不保真 然后你知道有人

2. 范围是多少？能“不管在哪里都能找到”吗？
不能，有效范围就是普通Wi-Fi信号的覆盖范围。

典型室内：无遮挡时可能几十米，隔一堵墙可能只剩10~20米，混凝土废墟中可能只有几米甚至完全被屏蔽。

室外空旷地带：配合高增益天线，最远能到几百米，但这已超出一般USB网卡的常规能力。

trackerjacker本身只是个软件，信号的接收距离完全取决于你的网卡和天线。它不是雷达，不能超视距探测，只能在你的硬件能“听到”这个设备的无线电信号时，才能发现它。

所以你说的“不管在哪里都能找到”是完全不成立的。它只是一个本地的无线电监听工具，作用范围等同于你的Wi-Fi能传多远。
